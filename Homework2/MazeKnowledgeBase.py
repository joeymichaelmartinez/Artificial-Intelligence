...
class MazeKnowledgeBase:
    ...
    def ask (self, query):
        ...
        for proposition, value in query.props.items():
            proposition_list = [((proposition[0], proposition[1]), not value)]
            not_query = MazeClause(proposition_list)
            clauses.add(not_query)

        while(True):    
            for clause_pair in itertools.combinations(clauses, 2):
                resolvents = MazeClause.resolve(*clause_pair)
